...
_PREROLL_BYTES = 320 * 60  # ~1.2s kept while waiting for speech to begin
...
async def speech_to_text_stream_asgi(scope, receive, send) -> None:
    ...
    try:
            ...
            if chunk:
                ...
                for off in range(0, len(chunk), _FRAME_BYTES):
                    ...
                    if (speech_ms >= MIN_SPEECH_SEC * 1000
                            and silence_ms >= END_SILENCE_SEC * 1000) or                             speech_ms >= MAX_UTTERANCE_SEC * 1000:
                        emitted = True
                        break
        ...
        _log.info(kv(stage="asr", call=call_id, frames=frames,
                     utterances=utterances, elapsed=time.perf_counter() - started))
        try:
